...
coffeeStands = [((l*(1/5)+l/20, 3*l/5),(l*(1/5)+3*l/20, l*3/5+l/20))]

class coffeeStand: 
    def __init__(self, pos_x = 0, pos_y = 0, charge=0, coffeeCupsLeft=50): 
        self.pos = np.array([pos_x, pos_y])
        self.charge = charge
        self.coffeeCupsLeft = coffeeCupsLeft
        
    def studentCoffee(self):
        if self.coffeeCupsLeft > 0: self.coffeeCupsLeft -= 1 # en student får en kaffekopp - en kopp mindre igjen på stand
        if self.coffeeCupsLeft == 0: self.charge = 0
        
pos_coffeeStands = [(3*l/10, 3*l/5+l/40), (7*l/10, 3*l/5+l/40)]
coffeeStands = []
for stand in pos_coffeeStands:
    coffeeStands.append(coffeeStand(stand[0],stand[1],charge=1, coffeeCupsLeft=10))
print(coffeeStands[1].pos)
for stand in coffeeStands:
    # alle kaffestands har bredde l/20 og lengde l/10
    rectangle = canvas.create_rectangle(((stand.pos[0]-l/20,stand.pos[1]-l/40), (stand.pos[0]+l/20,stand.pos[1]+l/40)), fill="white")
pos_coffeeStands = np.array(pos_coffeeStands)/pixelsize*s
coffeeStands = []
for stand in pos_coffeeStands:
    coffeeStands.append(coffeeStand(stand[0],stand[1],charge=1, coffeeCupsLeft=50))
...
print(l/20*(s/pixelsize),"størræls")
def initSim(): 
    global rad, people 
    ...
    for i in range(num): 
        ...
        if i >= np.floor(num/2): 
            ...
            people[i] = Person(pos_x=rnd.uniform(8, 10), pos_y=rnd.uniform(4.15, 5.85), vel_x=-1, vel_y=rnd.uniform(-.01, .01), gv_x=-3, 
                               gv_y=rnd.uniform(-.01, .01),zonetype=1,friendtype=0,coffeeCharge=1) 
        else: 
            people[i] = Person(pos_x=rnd.uniform(0, 2), pos_y=rnd.uniform(4.15,5.85), vel_x=1, vel_y=rnd.uniform(-.01, .01), gv_x=3, 
                               gv_y=rnd.uniform(-.01, .01),zonetype=1,friendtype=0,coffeeCharge=1) 
...
def coffeeForce(person, stand, scaling,r):
    print(stand.pos[0], person.pos[0])
    print("cups left:", stand.coffeeCupsLeft)
    if stand.pos[0]-l/20*(s/pixelsize) < person.pos[0] < stand.pos[0]+l/20*(s/pixelsize) and stand.pos[1]-l/40*(s/pixelsize) < person.pos[1] < stand.pos[1]+l/40*(s/pixelsize):
        person.setCoffeeCharge(0)
        stand.studentCoffee()
    r_vec = stand.pos-person.pos 
    dist = r_vec.dot(r_vec) 
    unit_vec = r_vec/dist 
    gradient = unit_vec*((-6/(dist-2*r)**7)+(12/(dist-2*r)**13))*person.coffeeCharge*stand.charge
    gradient_size = np.sqrt(gradient.dot(gradient)) 
    if gradient_size > maxF: 
        gradient = gradient*maxF/gradient_size # scaling down the gradient 
    return gradient/scaling

def find_closest_coffeeStand(person): # returnerer den standen som er nærmerst "person"
    global coffeeStands
    closest_stand = 0
    smallest_dist = 10000
    for stand in coffeeStands:
...
def update(dt): 
    global people, deleted_indexes, coffeeStands
    findNeighbors() 
    ...
    for i in range(len(people)):
        ...
        F[i] += FAvoid
        closest_coffee_stand = find_closest_coffeeStand(people[i])
        if closest_coffee_stand == coffeeStands[0]:
            print("LEFT STAND",coffeeStands[0].pos)
        elif closest_coffee_stand == coffeeStands[1]:
            print("RIGHT STAND",coffeeStands[1].pos)
            
        Fcoffee = coffeeForce(people[i], closest_coffee_stand, coffeeScale, rad)
        print("coffeeForce:", Fcoffee)
        F[i] += Fcoffee
